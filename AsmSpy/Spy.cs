...
namespace AsmSpy
{
    internal class Spy
    {
        ...
        public void Analyse()
        {
            ...
            AnalyseAssemblies(assemblyFiles, _options.SkipSystem, _options.All);
        }
        ...
        public static void AnalyseAssemblies(List<FileInfo> assemblyFiles, bool skipSystem, bool all)
        {
            ...
            foreach(var assembly in assemblies.OrderBy(x => x.Key))
            {
                ...
                if(!all && (assembly.Value.GroupBy(x => x.VersionReferenced).Count() == 1))
                    continue;
